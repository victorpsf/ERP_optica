...
WITH PERMISSION_TEMP AS (
    SELECT DISTINCT P1.PERMISSIONID
    ...
    INNER JOIN  PAPERXENTERPRISE    AS P4       ON  P4.PAPERID       = P3.PAPERID
    ...
    INNER JOIN  AUTH                AS A4       ON  A4.AUTHID        = A1.AUTHID
    INNER JOIN  CREDENTIAL          AS C1       ON  C1.CREDENTIALID  = A4.CREDENTIALID
    INNER JOIN  PERSON              AS P5       ON  P5.PERSONID      = C1.PERSONID

    INNER JOIN  AUTHXPAPER          AS A2       ON  A2.AUTHID        = A4.AUTHID        AND A2.PAPERID       = P3.PAPERID
    WHERE
