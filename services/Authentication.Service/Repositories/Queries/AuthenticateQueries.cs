...
";
    ...
    public static string ForgottenSql = @"
SELECT
	A1.AUTHID AS USERID,
    A1.NAME   AS NAME,
	C2.VALUE  AS EMAIL
...
INNER JOIN  CREDENTIAL 		AS C1 	ON C1.CREDENTIALID = A1.CREDENTIALID
INNER JOIN  PERSON 			AS P1 	ON P1.PERSONID = C1.PERSONID
INNER JOIN  PERSONCONTACT   AS C2   ON C2.PERSONID = P1.PERSONID
WHERE 
		A1.DELETED_AT IS NULL
	AND C1.DELETED_AT IS NULL
    AND P1.DELETED_AT IS NULL

	AND C2.CONTACTTYPE = @CONTACTTYPE
    AND (@NAME IS NOT NULL OR @EMAIL IS NOT NULL)
    AND (
			(@NAME IS NULL OR @NAME = A1.NAME)
        AND (@EMAIL IS NULL OR UPPER(@EMAIL) = UPPER(C2.VALUE))
    )
";

    public static string ForgottenChangePassphraseSql = @"
UPDATE AUTH
