...
public partial class SignInController: ControllerBase
{
    ...
    public SignInController(IBaseControllerServices baseControllerServices, AuthenticateRepoService service)
    {
    ...
    private AccountResultModels.ValidateInputResult ValidateInput<T>(AccountModels.SingInInput data, ControllerBaseModels.RequestResult<T> output)
    {
        var user = this.service.Find(
            new AuthenticateRules.SingInRule { 
                Login = data.Name ?? string.Empty, 
    ...
    [HttpPost]
    [AllowAnonymous]
    public IActionResult Index([FromBody] AccountModels.SingInInput data)
    {
